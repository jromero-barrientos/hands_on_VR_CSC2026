...
{
  // Usage:
  //   ./exampleB01 <mode> <numberOfEvents>
  // where:
  //   mode < 0 : analog (no biasing)
  //   mode = 0 : importance biasing
  //   mode > 0 : weight window biasing
  // If <numberOfEvents> is omitted, a default is used.
  ...
  if (argc > 2) {
    const G4int n = atoi(argv[2]);
    if (n > 0) numberOfEvents = n;
  }

  G4long myseed = 345354;

  // If no command-line arguments are given, run in interactive mode
  // with a UI session (this will typically be used together with a
  // visualization macro like "vis.mac").
  ...
  // mode < 0 (e.g. -1) is analog: no biasing physics registered
  ...
  // mode < 0 (e.g. -1): analog, no biasing stores created

  // Visualization manager (initialized as in example B1)
  G4VisManager* visManager = new G4VisExecutive(argc, argv);
  visManager->Initialize();

  //  runManager->BeamOn(numberOfEvents);

  // Use interactive UI session if available, otherwise run in batch mode
  G4UImanager* UImanager = G4UImanager::GetUIpointer();
  if (ui) {
    // Execute a visualization/control macro (e.g. vis.mac) and
    // start an interactive session. Users can then issue /run/beamOn
    // commands by hand.
    ...
    // Batch mode: keep the original behaviour
