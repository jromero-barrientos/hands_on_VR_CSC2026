    ...
    if (mfd) {
      ...
      for (G4int icol = 0; icol < mfd->GetNumberOfPrimitives(); icol++) {
        ...
        if (collectionID >= 0) {
          ...
          fRunMapSq.push_back(new G4THitsMap<G4double>(detName, collectionName));
...
void B01Run::RecordEvent(const G4Event* aEvent)
{
  ...
  for (G4int i = 0; i < Ncol; i++) {  // Loop over HitsCollection
    ...
    if (EvtMap) {
      ...
      *fRunMap[i] += *EvtMap;
      //======================================================
      // NEW: acumular suma de cuadrados en fRunMapSq (para varianza)
      if (fRunMapSq.size() == fRunMap.size() && fRunMapSq[i]) {
        // Recorremos el mapa de este evento
        std::map<G4int, G4double*>* evtMap = EvtMap->GetMap();
        for (auto itr = evtMap->begin(); itr != evtMap->end(); ++itr) {
          G4int copyNo    = itr->first;
          G4double value  = *(itr->second);
          G4double value2 = value * value;
          // Usamos add() para acumular el cuadrado
          // (G4THitsMap::add suma al valor existente o crea uno nuevo)
          fRunMapSq[i]->add(copyNo, value2);
        }
      }
    }
  }
...
//  By  MultiFunctionalDetector name and Collection Name.
G4THitsMap<G4double>* B01Run::GetHitsMapSq(const G4String& detName, const G4String& colName)
{
  G4String fullName = detName + "/" + colName;
  return GetHitsMapSq(fullName);
}
...
G4THitsMap<G4double>* B01Run::GetHitsMapSq(const G4String& fullName)
{
  G4int Ncol = fCollName.size();
  for (G4int i = 0; i < Ncol; i++) {
    if (fCollName[i] == fullName) {
...
void B01Run::Merge(const G4Run* aRun)
{
  ...
  for (G4int i = 0; i < nCol; i++) {  // Loop over HitsCollection
    ...
    // Merge squared sums
    if (fRunMapSq.size() == fRunMap.size() &&
        localRun->fRunMapSq.size() == localRun->fRunMap.size() &&
        fRunMapSq[i] && localRun->fRunMapSq[i]) {
      *(fRunMapSq[i]) += *(localRun->fRunMapSq[i]);
    }
  }
